(()=>{
  ...
  async function previewCsv(){const file=$('#reservationCsvFile').files[0];if(!file)return;csvRows=parseCsv(await file.text());const preview=$('#reservationCsvPreview'),invalid=csvRows.filter(row=>row.errors.length),valid=csvRows.filter(row=>!row.errors.length),ordered=[...invalid,...valid];preview.classList.remove('csv-preview-empty');preview.innerHTML=`<div class="csv-summary"><strong>${valid.length} válida(s)</strong><button id="showCsvErrors" type="button" ${invalid.length?'':'disabled'}>${invalid.length} com erro</button></div>`+ordered.slice(0,50).map(row=>`<div class="csv-row ${row.errors.length?'invalid':''}"><span>Linha ${row.line}</span><strong>${safe(row.professor||'Dados não identificados')}</strong><span>${safe(row.dateRaw)} → ${safe(row.date)} • ${safe(row.timeRaw)} • ${safe(row.subject)}</span><small>${row.errors.length?`Erro: ${safe(row.errors.join(', '))}`:safe(row.server.email)}</small></div>`).join('');$('#showCsvErrors')?.addEventListener('click',()=>preview.querySelector('.csv-row.invalid')?.scrollIntoView({behavior:'smooth',block:'center'}));$('#importReservationsCsv').disabled=!valid.length}
